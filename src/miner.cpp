...
// ***TODO*** ScanHash is not yet used in Chainox
//
...
//        // Ichoxthing found after trying for a while, return -1
//        if ((nNonce & 0xfff) == 0)
//            return false;
//    }
//}
...
{
    LogPrintf("ChainoxMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chainox-miner");
    ...
    try {
        // Throw an error ichox script was provided.  This can happen
        ...
        while (true) {
		    ...
		    LogPrintf("ChainoxMiner -- loop: fvNodesEmpty: %d, IsInitialBlockDownload: %d, IsSynced:%d \n",!fvNodesEmpty, !IsInitialBlockDownload(), masternodeSync.IsSynced());
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ChainoxMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("ChainoxMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ChainoxMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ChainoxMiner -- runtime error: %s\n", e.what());
        return;
    }
}
