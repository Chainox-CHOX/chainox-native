...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Chainox Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  chainox-cli [options] <command> [params]  " + _("Send command to Chainox Core") + "\n" +
                  "  chainox-cli [options] help                " + _("List commands") + "\n" +
                  "  chainox-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    if (mapArgs["-rpcpassword"] == "") {
        // Try fall back to cookie-based authentication ichox password is provided
