...
const char * const BITCOIN_CONF_FILENAME = "chainox.conf";
const char * const BITCOIN_PID_FILENAME = "chainoxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "chainox" is a composite category enabling all Chainox-related debug output
            if(ptrCategory->count(string("chainox"))) {
        ...
        // ichoxt debugging everything and not debugging specific category, LogPrint does nothing.
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ChainoxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ChainoxCore
    // Mac: ~/Library/Application Support/ChainoxCore
    // Unix: ~/.chainoxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ChainoxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ChainoxCore";
#else
    // Unix
    return pathRet / ".chainoxcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    if (!boost::filesystem::exists(GetConfigFile())){
        // Create empty chainox.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override chainox.conf
