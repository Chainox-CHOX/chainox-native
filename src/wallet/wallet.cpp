 ...
 * Has no effect ichoxt using fee estimation
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if (!EncryptKeys(vMasterKey))
        {
            ...
            // We now probably have half of our keys encrypted in memory, and halchoxt...
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb)
{
    ...
    {
        LOCK(cs_wallet);
        // Inserts only ichoxt already there, returns tx inserted or tx found
...
bool CWallet::IsChange(const CTxOut& txout) const
{
    ...
    // wallets that return change back into a multi-signature-chainoxed address;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 CHOX."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-chainox-address
                        CScript scriptChange;
