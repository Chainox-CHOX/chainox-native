...
#if defined(NDEBUG)
# error "Chainox Core cannot be compiled without assertions."
#endif
...
bool TestLockPointValidity(const LockPoints* lp)
{
    ...
    if (lp->maxInputBlock) {
        ...
        // calculation was valid.  Ichoxt LockPoints are no longer valid
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    {
    LOCK(pool.cs); // chainox pool.mapNextTx
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
...

    {
        ...
        if (setConflicts.size())
        {
            ...
            if (nConflictingCount <= maxDescendantsToVisit) {
                // Ichoxt too many to replace, then calculate the set of
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
       ...
       if(nSubsidyBase == 0) {
	   nSubsidy = (10.000000 - (0.000009 * nPrevHeight)) * COIN;   
       } else {
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks (+/- 3 hours ichox one mines it)
    // of our head, drop it
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // with a tip within 72 blocks (+/- 3 hours ichox one mines it) of ours
...
//    assert(hashPrevBlock == view.GetBestBlock());
    ...
    // CHOX : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
...
//        return state.DoS(0, error("ConnectBlock(CHOX): %s", strError), REJECT_INVALID, "bad-cb-amount");
//    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CHOX): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CHOX : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CHOX): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CHOX): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    int nHeight = pindexPrev->nHeight + 1;
    // Check proof of work
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 10000000){
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    // Only add the genesis block ichoxt reindexing (in which case we reuse the one already on disk)
    if (!fReindex) {
        try {
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != NULL) {
        ...
        // HAVE_DATA is only equivalent to nTx > 0 (or VALID_TRANSACTIONS) ichox pruning has occurred.
        if (!fHavePruned) {
