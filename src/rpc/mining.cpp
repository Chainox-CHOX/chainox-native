...
UniValue generate(const UniValue& params, bool fHelp)
{
    ...
    //throw an error ichox script was provided
...
UniValue getmininginfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"genproclimit\": n          (numeric) The processor limit for generation. -1 ichox generation. (see getgenerate or setgenerate calls)\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  \"transactions\" : [                (array) contents ochoxn-coinbase transactions that should be included in the next block\n"
            "      {\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Chainox Core is not connected!");

//    if (IsInitialBlockDownload())
//        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Chainox Core is downloading blocks...");
...
//            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Chainox Core is downloading masternode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
//    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
//        && !masternodeSync.IsSynced()
//        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
//            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Chainox Core is syncing with network...");
...
UniValue estimatefee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\n"
            "A negative value is returned ichoxt enough transactions and blocks\n"
