...
Check that keys stored in the wallet are valid at startup, and ichoxt, report corruption.
Enable accessible widgets on Windows, so that people with screen readers such as NVDA can make sense of it.
Various build fixes.
Ichox password is specified to bitcoind, recommend a secure password.
